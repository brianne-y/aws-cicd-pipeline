...
resource "aws_security_group" "alb" {
  ...
  description = "ALB: allow HTTP from internet inbound only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "ecs_tasks" {
  ...
  egress {
    ...
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
    description = "Outbound to ECR and CloudWatch via NAT Gateway"
  }
...
resource "aws_security_group_rule" "alb_to_ecs_egress" {
  ...
  description              = "ALB forwards to ECS tasks on port 5000 only"
}
...
resource "aws_security_group_rule" "ecs_from_alb_ingress" {
  ...
  description              = "ECS tasks accept port 5000 from ALB only"
}
...
# ── ALB, ECS Cluster, Task Definition, and Service ──
...
resource "aws_cloudwatch_metric_alarm" "alb_5xx" {
  ...
  alarm_description   = "ALB 5xx errors — investigate ECS task health and logs"
...
resource "aws_cloudwatch_metric_alarm" "ecs_tasks_low" {
  ...
  alarm_description   = "ECS running task count below 1 — service may be down"
  alarm_actions       = [aws_sns_topic.alerts.arn]
  ...
  tags = { Name = "${var.app_name}-ecs-tasks-low-alarm" }
}